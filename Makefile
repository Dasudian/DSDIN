...
test:
	$(eval EPOCH_PROCESSES := $(shell ps -fea | grep "bin/dsdin" | grep -v grep | wc -l))
...
docker:
	@docker build . -t dsdin/dsdin:local

ST_DOCKER_FILTER=--filter label=dsdin_system_test=true
...
smoke-test-run:
	@./rebar3 as system_test do ct --dir system_test --logdir system_test/logs --config system_test/cfg --suite=dsdst_sync_SUITE,dsdst_commands_SUITE

system-test:
	@./rebar3 as system_test do ct --dir system_test --logdir system_test/logs --config system_test/cfg $(CT_TEST_FLAGS)

system-test-deps:
#	docker pull dsdin/dsdin:latest
...
dsdvm-test: dsdvm-test-deps
	@./rebar3 eunit --application=dsdvm

dsdvm-test-deps: $(AEVM_EXTERNAL_TEST_DIR)/ethereum_tests
dsdvm-test-deps:
...
swagger-docs:
	(cd ./apps/dsdhttp && $(MAKE) swagger-docs);

swagger-check:
	./swagger/check \
		"$(CURDIR)/config/swagger.yaml" \
		"swagger" \
		"$(CURDIR)/apps/dsdhttp/priv/swagger.json" \
...
clean:
	@./rebar3 clean
	( cd apps/dsdsophia/test/contracts && $(MAKE) clean; )
...
distclean: clean
	( cd apps/dsdcuckoo && $(MAKE) distclean; )
...
multi-build: dev1-build
	...
	@for x in dev2 dev3; do \
		...
		cp config/$$x/sys.config _build/$$x/rel/dsdin/releases/$(VER)/sys.config; \
		cp config/$$x/vm.args _build/$$x/rel/dsdin/releases/$(VER)/vm.args; \
	done
...
internal-clean: $$(KIND)
	@rm -rf ./_build/$(KIND)/rel/dsdin/data/mnesia
...
.PHONY: \
	...
	dialyzer \
	docker docker-clean \
	test smoke-test smoke-test-run system-test system-test-deps dsdvm-test-deps\
	kill killall \
	clean distclean \
