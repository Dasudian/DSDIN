...
-include("dsdso_parse_lib.hrl").

-spec string(string()) -> {ok, [dsdso_syntax:decl()]} | {error, {dsdso_parse_lib:pos(), atom(), term()}}.
string(String) ->
  parse_and_scan(file(), String).

parse_and_scan(P, S) ->
  case dsdso_scan:scan(S) of
    {ok, Tokens} -> dsdso_parse_lib:parse(P, Tokens);
    Error        -> Error
  end.
...
-type ann()      :: dsdso_syntax:ann().
-type ann_line() :: dsdso_syntax:ann_line().
-type ann_col()  :: dsdso_syntax:ann_col().
...
-spec parse_pattern(dsdso_syntax:expr()) -> dsdso_parse_lib:parser(dsdso_syntax:pat()).
...
-spec parse_field_pattern(dsdso_syntax:field(dsdso_syntax:expr())) -> dsdso_parse_lib:parser(dsdso_syntax:field(dsdso_syntax:pat())).
...
-spec bad_expr_err(string(), dsdso_syntax:expr()) -> no_return().
bad_expr_err(Reason, E) ->
  ret_doc_err(get_ann(E),
              prettypr:sep([prettypr:text(Reason ++ ":"),
                            prettypr:nest(2, dsdso_pretty:expr(E))])).
