...
simple_contracts_test_() ->
    {foreach,
     ...
     [ {lists:concat(["Parse the ", Contract, " contract."]),
        ...
        || Contract <- [counter, voting, all_syntax, '05_greeter', dsdproof, multi_sig, simple_storage, withdrawal, fundme, dutch_auction] ]
    }.

parse_contract(Name) ->
    parse_string(dsdso_test_utils:read_contract(Name)).

roundtrip_contract(Name) ->
    round_trip(dsdso_test_utils:read_contract(Name)).
...
round_trip(Text) ->
    ...
    Text1     = prettypr:format(dsdso_pretty:decls(Contract)),
