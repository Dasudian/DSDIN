-module(dsdc_chain_metrics_probe).
...
probe_handle_msg({gproc_ps_event, block_created, #{time := T}},
    ...
    if LastT =:= undefined ->
       ...
       true ->
            ...
            dsdc_metrics:try_update([dsd,epoch,dsdcore,mining,interval], Diff)
    end,
...
probe_handle_msg({gproc_ps_event, top_changed, #{time := T}},
    ...
    if LastT =:= undefined ->
       ...
       true ->
            Diff = max(0, timer:now_diff(T, LastT) div 1000),
            lager:debug("Diff = ~p (LastT=~p", [Diff, LastT]),
            dsdc_metrics:try_update(
              [dsd,epoch,dsdcore,chain,top_change,interval], Diff)
    end,
...
total_difficulty() ->
    try dsdc_chain:difficulty_at_top_block() of
        {ok, V} -> V
    catch
        error:_ -> 0
    end.
