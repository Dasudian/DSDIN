...
%%% @doc Assembler for dsdvm machine code.
%%%
...
-include_lib("dsdbytecode/include/dsdb_opcodes.hrl").
...
format([Mnemonic | Rest], Address) ->
    Op = dsdb_opcodes:m_to_op(Mnemonic),
    case (Op >= ?PUSH1) andalso (Op =< ?PUSH32) of
        true ->
            Arity = dsdb_opcodes:op_size(Op) - 1,
...
file(Filename, Options) ->
    ...
    {ok, Tokens, _} = dsdb_asm_scan:scan(binary_to_list(File)),
...
to_bytecode([{mnemonic,_line, Op}|Rest], Address, Env, Code, Opts) ->
    OpCode = dsdb_opcodes:m_to_op(Op),
    OpSize = dsdb_opcodes:op_size(OpCode),
...
expand_args([OP, Arg | Rest]) when OP >= ?PUSH1 andalso OP =< ?PUSH32 ->
    BitSize = (dsdb_opcodes:op_size(OP) - 1) * 8,
    Bin = << << X:BitSize>> || X <- [Arg] >>,
