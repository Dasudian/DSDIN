...
rlp_one_byte_test() ->
    B = <<42>>,
    B = utils_rlp:encode(B),
    B = utils_rlp:decode(B).
...
rlp_two_bytes_test() ->
    ...
    <<S, B/binary>> = utils_rlp:encode(B).

rlp_one_byte_size_bytes_test() ->
    L = 55,
    S = ?BYTE_ARRAY_OFFSET + L,
    X = << <<X>> || X <- lists:seq(1,L)>>,
    E = <<S, X/binary>> = utils_rlp:encode(X),
    X = utils_rlp:decode(E).
...
rlp_tagged_size_one_byte_bytes_test() ->
    ...
    S = byte_size(X),
    E = <<Tag, S, X/binary>> = utils_rlp:encode(X),
    X = utils_rlp:decode(E).
...
rlp_tagged_size_two_bytes_bytes_test() ->
    ...
    X = list_to_binary(lists:duplicate(L, 42)),
    S = byte_size(X),
    E = <<Tag, S:SizeSize/unit:8, X/binary>> = utils_rlp:encode(X),
    X = utils_rlp:decode(E).
...
rlp_byte_array_list_test() ->
    ...
    X = lists:duplicate(L, <<42>>),
    Y = list_to_binary(X),
    E = <<Tag, Y/binary>> = utils_rlp:encode(X),
    X = utils_rlp:decode(E).
...
rlp_byte_array_tagged_size_one_byte_list_test() ->
    ...
    Y = list_to_binary(X),
    S = byte_size(Y),
    E = <<Tag, S:SizeSize/unit:8, Y/binary>> = utils_rlp:encode(X),
    X = utils_rlp:decode(E).
...
illegal_size_encoding_list_test() ->
    ...
    ?assertError(leading_zeroes_in_size, utils_rlp:decode(E1)).
